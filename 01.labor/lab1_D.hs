...
-- 1c. Multiply two numbers
mymult :: Num a => a -> a -> a
mymult a b = a * b

-- 1d. Divide two numbers
...
-- 1e. Get the remainder of an integer division
mymod :: Integral a => a -> a -> a
mymod a b = a `mod` b

-- 2. Calculate the root of a first degree polynomial (ax + b = 0)
root1 :: Fractional a => a -> a -> a
root1 a b = (-b) / a

-- 3. Get the absolute value of a number
...
-- 4. Get the sign of an integer (-1 for negative, 1 for positive)
...
-- 8. Are two value pairs nearly the same? Example:
--     (1,2) = (2,1) TRUE
--     (1,2) = (3,4) FALSE
...
-- 9. Calculate the factorial of an integer.
...
primes2 n = [i | i <- [2..n], primeL i]
    where
        primeL n = divL n == [1, n]
        divL n = [i | i <- [1..n], mod n i == 1]